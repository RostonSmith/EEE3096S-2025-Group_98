...
#define NUM_SCALABILITY_TESTS 12
/* USER CODE END PD */
...
const uint16_t scalability_widths[NUM_SCALABILITY_TESTS] = {
    64, 128, 256, 320, 480, 640,     // Direct processing range
    800, 1024, 1280, 1600, 1920, 1920  // Tiled processing required
};

const uint16_t scalability_heights[NUM_SCALABILITY_TESTS] = {
    64, 128, 256, 240, 270, 360,     // Direct processing range  
    450, 576, 720, 900, 1080, 1080   // Tiled processing required
};
...
void run_scalability_test(void) {
  for (uint8_t i = 0; i < NUM_SCALABILITY_TESTS; i++) {
    // Brief LED indication for each test
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, (i % 2) ? GPIO_PIN_SET : GPIO_PIN_RESET);
